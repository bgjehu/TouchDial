...
class Contact {
    ...
    var numberStringWithType : String {
        get{
            return number.stringByAppendingString(numberType != "" ? "(\(numberType))" : "")
        }
    }
    var oneliner : String {
        get{
            if name == "" {
                //  "123-456-7890"
                //  "123-456-7890(main)"
                return numberStringWithType
            } else {
                //  "JE: 123-456-7890"
                //  "JE: 123-456-7890(iPhone)"
                return "\(name): \(numberStringWithType)"
            }
        }
    }
    ...
    func toAppShortcutItem() -> UIApplicationShortcutItem {
        let shortCut = UIApplicationShortcutItem(type: "TouchDialContact", localizedTitle: name, localizedSubtitle: numberStringWithType, icon: nil, userInfo: self.toJSONRecord())
        return shortCut
    }
